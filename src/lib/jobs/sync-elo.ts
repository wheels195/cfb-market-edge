/**
 * Sync Elo ratings from CFBD into team_elo_snapshots
 *
 * Should run after each game day to keep ratings current.
 * During bowl season, games happen on various days so this
 * should be called as part of the main pipeline.
 */
...
export interface SyncEloResult {
  season: number;
...
/**
 * Build mapping from CFBD team name to our team ID
 */
...
/**
 * Sync Elo ratings for a specific season
 */
...
/**
 * Main sync function - syncs current season Elo ratings
 */
export async function syncElo(): Promise<SyncEloResult> {
  console.log('[SyncElo] Starting Elo sync...');
